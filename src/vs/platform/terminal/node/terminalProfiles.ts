...
async function getWslProfiles(wslPath: string, defaultProfileName: string | undefined): Promise<ITerminalProfile[]> {
	...
	const distroOutput = await new Promise<string>((resolve, reject) => {
		...
		// Using execFile with absolute wslPath prevents Untrusted Search Path (PATH hijacking) vulnerability
		cp.execFile(wslPath, ['-l', '-q'], { encoding: 'utf16le', env: { ...process.env, WSL_UTF8: '0' }, timeout: 1000 }, (err, stdout) => {
			if (err) {
