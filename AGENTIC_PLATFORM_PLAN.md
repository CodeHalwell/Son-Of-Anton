...
- [x] **F-3** Per-agent retrieval ranking weights. _(partial: implemented for `services/mcp-gateway/src/tools/semanticSearch.ts` — the `semanticSearch.ts` named in this finding — via `.son-of-anton/retrieval-weights.json` keyed by agent handle, resolved per call from a new `agentRole` input on `semantic_search`. Does **not** apply when `sota.codeGraph.backend` resolves to the embedded `services/code-graph/mcp-server` (the default under `auto` when no Docker stack is detected): that backend calls the Rust engine directly with no semantic/structural split to weight. Extending weighting to the embedded backend is tracked as F-28.)_
- [ ] **F-28** Per-agent retrieval weighting for the embedded code-graph backend. `services/code-graph/mcp-server` (Rust engine via `crates/sota-codegraph-core`) has no equivalent to `mcp-gateway`'s semantic/structural hybrid score, so F-3's per-agent weights don't apply when `sota.codeGraph.backend` is `embedded` or resolves there via `auto` — the default extension configuration. Needs either a comparable weighted-scoring pass in the Rust engine or routing `semantic_search` through `mcp-gateway` regardless of backend choice.
- [ ] **F-29** `semantic_search` candidate pool is sized off semantic similarity alone (`qdrant.search(queryVector, maxResults * 2, filter)` in `semanticSearch.ts`), so a structural-heavy `RetrievalWeights` (F-3) can only re-rank within that window — a highly-referenced symbol Qdrant's top-2N semantic matches don't include is never a candidate, no matter how much structural weight an agent role configures. Needs either an adaptive candidate-pool size keyed off the weight split, or a second FalkorDB-sourced candidate set (e.g. top in-degree symbols) merged in before scoring.
