...
## 2024-05-01 - Untrusted Search Path Vulnerability in WSL Executable
**Vulnerability:** Found `cp.exec('wsl.exe -l -q', ...)` which executes `wsl.exe` based on the system PATH.
**Learning:** `cp.exec` evaluates commands in a shell and searches for the executable in the `$PATH` environment variable. A local attacker could modify the PATH to point to a malicious executable named `wsl.exe`, leading to arbitrary code execution (Untrusted Search Path / PATH Hijacking).
**Prevention:** Use `cp.execFile` and provide the fully qualified path to the executable rather than just the executable name. In this case, `wslPath` was passed as an argument and can be used directly: `cp.execFile(wslPath, ['-l', '-q'], ...)`.
