...
## 2026-07-01 - Array.indexOf in Array.filter optimizations
**Learning:** Repeatedly calling array.indexOf inside array.filter on large datasets creates an O(N*M) bottleneck.
**Action:** Convert arrays to Sets and use Set.has() to reduce time complexity to O(N+M) when comparing configuration keys.
