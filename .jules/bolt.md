...
## 2026-09-01 - O(N*M) nested iterations in array filtering using indexOf
**Learning:** Checking for element presence with `Array.prototype.indexOf()` or `Array.prototype.includes()` inside an `Array.prototype.filter()` callback over another array (such as editor contributions or extensions) creates an O(N*M) time complexity bottleneck. This occurs frequently when configuring extensions or editor widgets, blocking the main thread when many elements are involved.
**Action:** When filtering an array against an exclusion or inclusion list, always convert the target array to a `Set` first, and use `Set.has()` to ensure O(N+M) time complexity.
