...
## 2026-07-23 - O(N*M) nested iterations in array filtering for recursive extension enablement checks
**Learning:** Checking for element presence with `Array.prototype.indexOf()` inside an `Array.prototype.filter()` callback over the `checked` tracking array (passed by reference in `getExtensionsToEnableRecursively`) creates an O(N*M) time complexity bottleneck. This occurs in extension resolution logic during enablement operations, blocking the main thread when many elements are involved.
**Action:** When filtering an array against a tracking array in recursive calls, convert the tracking array to a local `Set` within the function to use `Set.has()`. This reduces time complexity to O(N+M).
