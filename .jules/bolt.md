...
## 2024-05-31 - O(N*M) lookups when comparing Sets and configuration models
**Learning:** Found nested iterations (O(N*M)) in `diffSets` array usage within `src/vs/workbench/api/common/extHostEditorTabs.ts` (`diff.removed.includes`) and user data sync profiles (`local.filter(({ id }) => !ignored.includes(id))`). When the source arrays are large, nested `.includes()` calls within `.filter()` or `.map()` create main thread blocking bottlenecks.
**Action:** When working with differences generated by sets (like `diffSets`), or comparing large arrays against an exclusion list, convert the target array into a `Set` first, and use `Set.has()` to ensure O(N+M) time complexity.
## 2026-08-10 - O(N*M) nested iterations in array filtering for recursive extension checking
**Learning:** Checking for element presence with \`Array.prototype.indexOf()\` or \`includes\` over a growing tracking array (passed by reference in a recursive function like \`getExtensionsRecursively\`) creates an O(N*M) time complexity bottleneck. This occurs in extension resolution logic, blocking the main thread when many elements are involved. Recreating a new Set out of the tracking array on every recursive call results in an unnecessary O(N) allocation overhead per call.
**Action:** When maintaining tracking collections in recursive functions, initialize a \`Set\` at the entry point and pass it down through the recursive calls. Explicitly mirror any modifications (e.g., \`Array.push()\`) to the tracked array with \`Set.add()\` within the recursive loop to maintain state and true O(1) lookups, reducing overall time complexity to O(N).
...
## 2026-08-23 - O(N*M) lookups in Workbench extension UI and editor contributions
**Learning:** Found nested iterations O(N*M) where `.includes()` or `.indexOf()` is called inside `.filter()` during workbench configurations (like `searchEditor.ts`, `replEditor.ts`, `extensionsCompletionItemsProvider.ts`, `extensionsViewlet.ts`). These functions block the main thread unnecessarily when generating view lists and contributions.
**Action:** Always wrap target arrays in a `Set` before passing them into `.filter(x => !array.includes(x))` or `.filter(x => array.indexOf(x) === -1)` structures, reducing time complexity to O(N+M) with `Set.has()`. Ensure to apply `Set.has()` optimizations to workbench UI components to prevent UI thread blocking.
