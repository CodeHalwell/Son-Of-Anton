...
Each specialist except the coordinating `anton` supports an `acpAgent` assignment. An empty assignment routes `claude-code-*` models through the configured `claude-acp` adapter and otherwise uses the native implementation. Adapter definitions and assignments are resolved on each turn; changing them does not require a window reload. Commands are user/machine settings; execution requires a trusted workspace. The CLI uses its existing workspace trust decision (`SOTA_TRUST_WORKSPACE=1`) and the run's approval policy. Unattended CLI surfaces without an approval gate deny permission requests. The IDE offers one-time permission choices in a modal with the proposed tool details.

Optional `env` supplies environment overrides; prefer inherited provider credentials over committing secrets to config. Optional `authMethodId` invokes only that explicitly configured, advertised method after initialization. Otherwise the adapter uses its existing login state. Custom external agents own their model configuration. The canonical `claude-acp` adapter receives the selected Claude Code tier through `ANTHROPIC_MODEL`. Unpinned specialists inherit the orchestrator's Claude Code provider while retaining their own tier (for example, Sonnet for code and Haiku for docs); pinned direct-API models remain native. The approved plan preserves both its provider selection and original editor context for specialist execution and review.
...
Use **Anton: Configure Claude ACP**, or **Connect via Claude Code** in chat settings. Confirm the registry's pinned package and launch command. The adapter reuses the existing Claude sign-in; specialist tools require a trusted workspace and retain the IDE's permission prompts. Node and the package runner must be installed on the host. Cancelling adapter setup leaves connection setup incomplete.

A missing adapter produces an actionable setup error before a specialist reaches the text-only transport. After configuration, retry with a fresh plan. Builds predating this routing change need explicit `sota.agents.<handle>.acpAgent` assignments and a window reload; an existing plan from an older build cannot recover editor context that was never captured.
...
The installed Gemini CLI 0.46.0 advertises `--acp`, but its live initialization did not complete during the bounded local probe; it is not marked interoperable from that probe. On 2026-09-08, Claude ACP 0.75.1 completed an authenticated read-only turn against the disposable test workspace: it used a real Read File tool and returned the README heading with `end_turn`. This verifies the local Claude sign-in and file-read path. The Codex adapter’s authenticated turns remain unverified; this Claude probe does not establish cross-platform interoperability.
