...
| History search changes the active conversation title | Filter out the active conversation; the title changes to “New Conversation” even though the transcript remains active. | Send active conversation metadata independently of search results, without loading the transcript. |
...
Core compilation, extension and webview type checks, built-in extension compilation and the extension bundle completed successfully. The full runs passed 404 core tests, 680 extension tests and 77 browser UI tests (1,161 total). The affected model-selection cases were rerun after the final label refinement. Browser fixtures exercise the shipped webview code; they do not replace the live native model rerun called out above.
