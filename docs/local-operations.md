...
## Legacy service checkpoints

The optional checkpoint service accepts older records that lack `workspaceRoot`. It validates every relative path and snapshot hash, reads the current recovery contents, and persists the configured workspace’s canonical identity before modifying files. A failed validation leaves both the record and workspace unchanged. Once bound, the checkpoint cannot be restored through a different workspace. This migration applies to the service’s file snapshots; the editor’s newer checkpoint journal and recovery pinning remain in place.
