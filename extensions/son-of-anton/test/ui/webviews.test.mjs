...
async function post(page, message) { await page.evaluate(data => window.dispatchEvent(new MessageEvent('message', { data, origin: window.origin, source: null })), message); }
...
test('chat: a new conversation clears the previous response token and cost totals', async t => {
	const page = await openSurface(t, 'chat', 420);
	await post(page, { type: 'messageComplete', totalTokens: 1234, estimatedCost: '1.25' });
	await post(page, { type: 'conversationCleared', conversationId: 'fresh' });
	assert.equal(await page.locator('#tokenCount').textContent(), '0 tokens');
	assert.equal(await page.locator('#costEstimate').textContent(), '$0.00');
});
...
test('streaming: drafting a follow-up cannot accidentally stop the current response', async t => {
	...
	await page.locator('#sendBtn').click();
	assert.equal(await page.evaluate(() => sentMessages.some(message => message.type === 'cancelRequest')), true);
	await post(page, { type: 'toolCall', id: 'cancel-test', name: 'read_file', status: 'running', input: {} });
	await post(page, { type: 'requestSettled', cancelled: true });
	await page.locator('.response-status').getByText('Response Stopped', { exact: true }).waitFor();
	assert.equal(await page.locator('[data-tool-status="running"]').count(), 0);
...
test('roster and specialist models: every specialist can be selected and every model override is wired', async t => {
	...
	for (const label of labels) {
		...
		assert.equal(await page.evaluate(() => sentMessages.findLast(message => message.type === 'selectSpecialist').specialistId), label.replace('Talk to @', ''));
		if (label === 'Talk to @anton-docs') assert.match(await page.locator('#messageInput').getAttribute('placeholder'), /Anton Docs/);
...
test('chat checklist follows board completion after the planning response settles', async t => {
	const page = await openSurface(t, 'chat', 420);
	await page.locator('#messageInput').fill('Plan the repair'); await page.locator('#messageInput').press('Enter');
	const task = { id: 'saved-task', assignee: 'anton-code', instruction: 'Fix totals', scopeFiles: ['cart.js'], dependencies: [], state: 'ready' };
	await post(page, { type: 'agentPlan', plan: { subtasks: [task] } });
	await post(page, { type: 'messageComplete' });
	await post(page, { type: 'tasksSnapshot', tasks: [{ ...task, state: 'done' }], counts: { done: 1 } });
	assert.equal(await page.locator('.plan-checklist-item').getAttribute('data-state'), 'done');
	assert.equal(await page.locator('.plan-checklist [data-progress]').textContent(), '1 / 1 done');
});
