...
interface TestSession {
	...
	llmClient: { getTokenUsage(): { input: number; output: number; cached: number }; estimateCost(): number };
}
...
suite('Chat turn ownership', () => {
	...
	test('new conversation usage excludes earlier requests from the shared client', async () => {
		const fixture = createSession();
		let input = 1000, output = 500, cost = 2;
		fixture.session.llmClient.getTokenUsage = () => ({ input, output, cached: 0 });
		fixture.session.switchConversation('second');
		fixture.session.costReporter = { getTotalCost: () => cost };
		const request = fixture.send('second conversation request');
		await request.ready;
		input += 20; output += 3; cost += 0.05;
		request.emit({ type: 'token', token: 'Done' }); request.release(); await request.done;
		const complete = fixture.messages.find(message => message.type === 'messageComplete');
		assert.deepEqual({ input: complete?.inputTokens, output: complete?.outputTokens, total: complete?.totalTokens, cost: complete?.estimatedCost }, { input: 20, output: 3, total: 23, cost: '0.0500' });
	});
