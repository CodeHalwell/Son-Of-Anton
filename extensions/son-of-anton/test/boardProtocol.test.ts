...
suite('Task board message boundary', () => {
	test('rejects invalid states and malformed chat messages before host execution', () => {
		const messages = [null, { type: 'dispatch', conversationId: 7, taskId: 'a' }, { type: 'board-action', action: 'setCardStatus', cardId: 'a', toColumn: 'not-a-state' }, { type: 'chat-runtime', requestId: 'r', model: 'sonnet', messages: 'oops' }, { type: 'chat-runtime', requestId: 'r', model: 'sonnet', messages: [{ role: 'user', content: {} }] }];
		assert.deepEqual(messages.map(isWebviewToHostMessage), [false, false, false, false, false]);
	});
