...
const options = JSON.parse(process.argv[2]) as { root: string; storage: string; mode: 'capture' | 'pin' | 'restore' | 'lock'; conversationId?: string; checkpointId?: string; crash?: boolean; snapshot?: FileSnapshot; legacy?: Checkpoint[] };
...
async function run(): Promise<void> {
	...
	if (options.mode === 'lock') {
		await withCheckpointLock(path.join(options.storage, 'lock-test'), async () => { send({ stage: 'ready' }); await wait(); if (options.crash) { process.exit(0); } });
		...
		send({ stage: 'captured', checkpoint }); manager.dispose();
	}
	process.disconnect();
}
