...
import { abortError, type AcpAgentDefinition, type AcpMcpServer, type AcpPermissionHandler, type AcpPromptResult, type AcpUpdate } from './protocol';
...
export class AcpRuntime {
	...
	constructor(options: { maxProcesses?: number; maxQueue?: number; idleTimeoutMs?: number } = {}) {
	...
	private key(turn: AcpTurn): string {
		...
		const fingerprint = createHash('sha256').update(JSON.stringify([turn.agent, turn.mcpServers ?? [], turn.modeId])).digest('hex');
	...
	private async execute(worker: Worker, job: Job): Promise<void> {
		try {
			...
			if (fresh) { await worker.connection.newSession(job.turn.mcpServers, job.controller.signal, job.turn.modeId); worker.ready = true; }
			...
			this.retire(worker);
			job.finish(job.controller.signal.aborted ? job.controller.signal.reason as Error : error as Error);
		} finally {
