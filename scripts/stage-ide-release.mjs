...
for (const target of expected) {
	...
	const report = JSON.parse(await readFile(path.join(source, 'installation-report.json'))); assert.equal(report.success, true); assert.equal(report.commit, manifest.commit);
...
await writeFile(path.join(output, 'build-manifest.json'), JSON.stringify({ version: 1, ideVersion: version, commit: process.env.GITHUB_SHA, builds }, null, 2) + '\n');
