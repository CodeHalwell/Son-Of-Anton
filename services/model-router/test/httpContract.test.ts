...
process.env.SOTA_SERVICE_TOKEN = 'http-contract-test-token';
const authorization = { Authorization: 'Bearer http-contract-test-token' };
...
async function fixture(t: TestContext, upstream: typeof fetch, timeoutMs = 1000) {
	...
	return {
		post: (body: object, endpoint = '/v1/messages') => clientFetch(url + endpoint, { method: 'POST', headers: { ...authorization, 'Content-Type': 'application/json' }, body: JSON.stringify(body) }),
		metrics: async () => (await clientFetch(url + '/metrics/recent', { headers: authorization })).json() as Promise<Array<{ success: boolean; inputTokens: number; outputTokens: number; cachedTokens: number }>>,
	};
}
