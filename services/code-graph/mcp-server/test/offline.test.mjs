...
async function connect(t, fixture, extra = []) {
...
test('installed native graph indexes, embeds, searches, watches edits and isolates workspaces offline', { timeout: 45000 }, async t => {
	...
	const live = await connect(t, app, [`--provider-embedder=http://127.0.0.1:${provider.address().port}/embeddings|fixture-v1|3`]);
	...
	const restarted = await connect(t, app);
	...
	const other = await connect(t, { ...app, workspace: otherWorkspace });
...
test('structural-only and missing-native installs advertise accurate capabilities', { timeout: 30000 }, async t => {
	const app = await fixture(t);
	const structural = await connect(t, app);
	...
	const broken = await connect(t, app);
